...
(function() {
  ...
  angular.module('oauth.iocare', ['oauth.utils'])
    .factory('$ngCordovaIocare', iocare);

  function iocare($q, $http, $cordovaOauthUtility) {
    return { signin: oauthIocare };

    /*
     * Sign into the iocare service
     *
     * @param    string clientId
     * @param    array appScope
     * @param    object options
     * @return   promise
     */
    function oauthIocare(clientId, appScope, options) {
      var deferred = $q.defer();
      if(window.cordova) {
        if($cordovaOauthUtility.isInAppBrowserInstalled()) {
          var redirect_uri = "http://localhost/callback";
          ...
          var flowUrl = "https://alexa.iocare.in/oauth/authorize?client_id=" + clientId + "&redirect_uri=" + redirect_uri + "&response_type=token&scope=" + appScope.join(",");
          if(options !== undefined && options.hasOwnProperty("auth_type")) {
            flowUrl += "&response_type=" + options.auth_type;
          }
          var browserRef = window.cordova.InAppBrowser.open(flowUrl, '_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
          browserRef.addEventListener('loadstart', function(event) {
            if((event.url).indexOf(redirect_uri) === 0) {
              ...
              if(parameterMap.access_token !== undefined && parameterMap.access_token !== null) {
                ...
                if ((event.url).indexOf("error_code=100") !== 0) {
                  deferred.reject("Iocare returned error_code=100: Invalid permissions");
                } else {
                  deferred.reject("Problem authenticating");
                }
              }
  ...
  iocare.$inject = ['$q', '$http', '$cordovaOauthUtility'];
})();
...
(function() {
  ...
  function cordovaOauth(
    ...
    $ngCordovaGoogle, $ngCordovaGithub, $ngCordovaFacebook,$ngCordovaIocare, $ngCordovaLinkedin, $ngCordovaInstagram, $ngCordovaBox, $ngCordovaReddit, $ngCordovaSlack,
